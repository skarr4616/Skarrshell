...
void ls(char *prompt)
{
    ...
    // Sorts the arguments for better parsing
    qsort(cmd.str_vec + 1, cmd.num - 1, sizeof(const char *), myCompare);
    ...
    if (n == -1)
    {
        ...
        if (errno == ENOTDIR)
        {
            ...
            else
            {
                ...
                struct command gmt_time = getArgv(ctime(&info.st_mtime));
    ...
    while (i < n)
    {
        ...
        else
        {
            if (info_all)
            {
                ...
                struct command gmt_time = getArgv(ctime(&info[i].st_mtime));
                printf("%s %s %.5s", gmt_time.str_vec[1], gmt_time.str_vec[2], gmt_time.str_vec[3]);
                printf(" ");
            }
